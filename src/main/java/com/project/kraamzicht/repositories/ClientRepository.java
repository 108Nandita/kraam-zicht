...
public interface ClientRepository extends JpaRepository<Client, String> {
    ...
    @Query("UPDATE Client a SET a.name = :#{#userDetailsDto.name}, a.surname = :#{#userDetailsDto.surname}, a.dob = :#{#userDetailsDto.dob}, a.address = :#{#userDetailsDto.address}, a.postalcode = :#{#userDetailsDto.postalcode}, a.place = :#{#userDetailsDto.place} WHERE a.username = :username")
    @Modifying
    void updateUserDetails(@Param("username") String username, @Param("userDetailsDto") UserDetailsDto userDetailsDto);

    @Query("UPDATE Client a SET a.phoneNr = :#{#contactDetailsDto.phoneNr}, a.email = :#{#contactDetailsDto.email} WHERE a.username = :username")
    @Modifying
    void updateContactDetails(@Param("username") String username, @Param("contactDetailsDto") ContactDetailsDto contactDetailsDto);

    @Modifying
    @Query("DELETE FROM Client a WHERE a.username = :username")
    void deleteClientByUsername(@Param("username") String username);
